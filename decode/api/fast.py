...
model_emotion = load_model(f'{DIR_MODELS}/Trained_InceptionResNetV2_resize_3')
...
async def predict(image: UploadFile=File()):
    ...
    with open("cv2_img.pkl", "wb") as file: pickle.dump(cv2_img, file)
    eq_img = histogram_equalization(cv2_img)
    with open("eq_img.pkl", "wb") as file: pickle.dump(eq_img, file)
